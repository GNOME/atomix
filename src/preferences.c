...
void handle_global_checkbox_toggled(GtkToggleButton *togglebutton, gpointer data);

void preferences_init(void)
{
	preferences = g_malloc(sizeof(Preferences));
	preferences->mouse_control = gnome_config_get_bool("/atomix/Control/MouseControl=true");
	preferences->keyboard_control = gnome_config_get_bool("/atomix/Control/KeyboardControl=false");
	preferences->hide_cursor = gnome_config_get_bool("/atomix/Control/HideCursor=true");
	preferences->lazy_dragging = gnome_config_get_bool("/atomix/Control/LazyDragging=true");
	preferences->mouse_sensitivity = gnome_config_get_int("/atomix/Control/MouseSensitivity=8");
	preferences->score_time_enabled = gnome_config_get_bool("/atomix/Global/EnableScoreAndTimeLimit=true");
}
...
void preferences_save(void)
{
	if(preferences)
	{
		...
		gnome_config_set_bool("/atomix/Control/MouseControl", preferences->mouse_control);
		gnome_config_set_bool("/atomix/Control/KeyboardControl",preferences->keyboard_control);
		gnome_config_set_bool("/atomix/Control/HideCursor",preferences->hide_cursor);
		gnome_config_set_bool("/atomix/Control/LazyDragging",preferences->lazy_dragging);
		gnome_config_set_int("/atomix/Control/MouseSensitivitiy",preferences->mouse_sensitivity);
		gnome_config_sync();
	}
}
...
void preferences_show_dialog(void)
{
	...
	check_lazy_dragging = glade_xml_get_widget (pref_xml, "check_lazy_dragging");
	...
	gtk_adjustment_set_value(GTK_ADJUSTMENT(adjustment), (preferences->mouse_sensitivity/2));
...
void handle_apply(GtkWidget *propertybox, gint page_num, gpointer data)
{
	...
	else if (page_num == 1 /* control */)
	{
		...
		preferences->lazy_dragging =
			gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(check_lazy_dragging));
...
void handle_global_checkbox_toggled(GtkToggleButton *togglebutton, gpointer data)
{
	if(get_game_state() != GAME_NOT_RUNNING)
	{
		...
		gtk_signal_disconnect_by_func(GTK_OBJECT(togglebutton), 
					      GTK_SIGNAL_FUNC(handle_global_checkbox_toggled),
					      data);
