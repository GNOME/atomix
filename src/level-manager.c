...
static void 
level_manager_class_init (GObjectClass *class)
{
...
static void 
level_manager_init (LevelManager *tm)
{
...
static void 
level_manager_finalize (GObject *object)
{
...
static void
create_level_sequence (LevelManager *lm, gchar *file)
{
	xmlDocPtr doc;
	xmlNodePtr node;
...
void 
level_manager_init_levels (LevelManager *lm)
{
	gchar* dir_path;
	gchar *sequence_file;
...
static gchar*
lookup_level_name (gchar *filename)
{
	xmlDocPtr doc;
	xmlNodePtr node;
	gchar* name = NULL;
...
static void 
add_level (LevelManager *lm, gchar *levelname, gchar *filename)
{
	gchar *search_result;
...
GList* 
level_manager_get_available_levels (LevelManager *lm)
{
	GList *list = NULL;
	
	g_return_val_if_fail (IS_LEVEL_MANAGER (lm), NULL);
		
	g_hash_table_foreach(lm->priv->levels,
			     (GHFunc) add_level_to_list, &list);
			     
	return list;
}

static Level*
load_level (gchar *filename)
{
	xmlDocPtr doc;
	xmlNodePtr node;
	Level *level = NULL;
